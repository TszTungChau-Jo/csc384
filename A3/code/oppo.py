...
def minimax_max_node(board, current_player, limit, caching = 0): #returns highest possible utility
    ...
    if len(possible_moves) == 0 or (limit == 0):  # Check if the game is over
        return None, compute_heuristic(board, current_player)
...
def sort_moves_by_utility(board, possible_moves, current_player, reverse_flag):
    ...
    move_utilities = [(move, compute_heuristic(play_move(board, current_player, move[0], move[1]), current_player)) for move in possible_moves]
...
def run_ai():
    ...
    """
    print("OppO AI") # First line is the name of this AI
